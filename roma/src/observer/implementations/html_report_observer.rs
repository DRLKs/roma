...
"#
    ...
    fn build_report_model(&self, report_path: &std::path::Path) -> ReportModel {
        ...
        let (status_text, status_class, status_note) = if finished {
            ("Completed".to_string(), "status-completed", None)
        } else {
            (
    ...
    fn render_html_document(&self, model: &ReportModel, chart_svg: &str) -> String {
        let status_note_box = model
            .status_note
            .as_ref()
            .map(|message| {
                format!(
                    "<div class=\"error-box\"><strong>Run note:</strong> {}</div>",
                    message
                )
            })
            .unwrap_or_default();
...
"#,
            ...
            status_text = model.status_text,
            status_note_box = status_note_box,
...
mod tests {
    ...
    fn observer_state(seq_id: u64, iteration: usize, presentation: &str) -> ObserverState {
        ObserverState::new(
            seq_id,
            iteration,
            (iteration + 1) * 10,
            10.0 - iteration as f64,
            7.0 - iteration as f64,
            3.0 - iteration as f64,
            presentation.to_string(),
        )
    }

    #[test]
    fn generates_report_on_end_with_summary_and_chart() {
        ...
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(0, 1, "selected=2/3"),
        });
        ...
        assert!(contents.contains("selected=2/3"));
        assert!(contents.contains("<svg"));
    }

    #[test]
    fn ignores_duplicate_and_out_of_order_snapshot_sequences() {
        let base = std::env::temp_dir().join(format!(
            "roma_html_report_dedup_test_{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .map(|d| d.as_nanos())
                .unwrap_or(0)
        ...
        let mut observer = HtmlReportObserver::new(base);
        observer.update(&AlgorithmEvent::<bool>::Start {
            algorithm_name: "DedupAlgorithm".to_string(),
        });
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(1, 1, "first"),
        });
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(1, 2, "duplicate"),
        });
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(0, 0, "stale"),
        });
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(2, 3, "second"),
        });

        assert_eq!(observer.generations.len(), 2);
        assert_eq!(observer.best_snapshots.len(), 2);
        assert_eq!(observer.generations[0].generation, 1);
        assert_eq!(observer.generations[1].generation, 3);
        assert_eq!(observer.best_snapshots[0].variables_preview, "first");
        assert_eq!(observer.best_snapshots[1].variables_preview, "second");
    }

    #[test]
    fn sanitize_folder_component_normalizes_symbols_and_defaults_when_empty() {
        assert_eq!(
            HtmlReportObserver::sanitize_folder_component("My GA/Experiment #1"),
            "my_ga_experiment_1"
        );
        assert_eq!(
            HtmlReportObserver::sanitize_folder_component("____"),
            "algorithm"
        );
        assert_eq!(
            HtmlReportObserver::sanitize_folder_component("A__B///C"),
            "a_b_c"
        );
    }
    ...
    fn report_uses_professional_css_and_semantic_sections() {
        ...
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(0, 1, "presentation=1"),
        });
        observer.update(&AlgorithmEvent::<bool>::End {
            total_generations: 1,
            total_evaluations: 5,
    ...
    fn report_shows_interrupted_status_when_finalize_runs_without_end() {
        ...
        observer.update(&AlgorithmEvent::<bool>::ExecutionStateUpdated {
            state: observer_state(0, 1, "selected=1/4"),
        });

...
IN_CONTAINER_ENV = "ROMA_KNAPSACK_PSO_IN_CONTAINER"
DOCKER_IMAGE = "roma-knapsack-pso:latest"
CONTAINER_RESULTS_DIR = "/workspace/benchmark_suite/knapsack_pso/results"
...
def build_runners():
    if os.environ.get(IN_CONTAINER_ENV) == "1":
        roma_command = ["/usr/local/bin/roma_knapsack_pso_benchmark"]
    else:
    ...
    return {
        "roma": roma_command,
        "pyswarms": [sys.executable, str(RUNNERS_DIR / "python" / "pyswarms_runner.py")],
        "exact_dp": [sys.executable, str(RUNNERS_DIR / "python" / "exact_dp_runner.py")],
...
def knapsack_cost(solution):
    ...
    total_weight = 0.0
    total_value = 0.0
    for bit, weight, value in zip(solution, weights, values):
        selected = 1 if isinstance(bit, bool) and bit else int(bit)
        if selected:
            total_weight += float(weight)
            total_value += float(value)

    if total_weight > capacity:
        return -(total_weight - capacity) * penalty
    ...
    for value in raw_solution:
        ...
        if isinstance(value, float) and abs(value - round(value)) <= 1e-9:
            ...
            if integer_value not in (0, 1):
                raise ValueError(f"solution float value {value!r} must round to 0 or 1")
            solution.append(bool(integer_value))
            continue
        raise ValueError(f"solution value {value!r} is not a boolean or 0/1 integer")
    return solution


def validate_results(results, library):
    ...
    known_optimum = float(INSTANCE["known_optimum"])
    ...
    for index, result in enumerate(results):
        ...
        if library == "exact_dp" and abs(float(result["best_fitness"]) - known_optimum) > 1e-6:
            invalid_runs += 1
            errors.append(
                f"run {index}: exact_dp should reach known optimum {known_optimum}, got {result['best_fitness']}"
            )
...
def summarize_optimality_gap(results):
    known_optimum = float(INSTANCE["known_optimum"])
    ok_results = [result for result in results if result.get("status") == "ok"]
    if not ok_results:
        return {"best_gap": None, "mean_gap": None}

    fitness_values = [float(result.get("best_fitness")) for result in ok_results]
    gaps = [known_optimum - fitness for fitness in fitness_values]
    return {
        "best_gap": min(gaps),
...
def main():
    ...
    summary = {
        ...
        "instance": {
            "problem": INSTANCE["problem"],
            "instance_id": INSTANCE["instance_id"],
            "instance_family": INSTANCE.get("instance_family"),
    ...
    for library, command in runners.items():
        ...
        validation = validate_results(results, library)
        aggregate = summarize_results(results, objective_sense="max")
        optimality_gap = summarize_optimality_gap(results)
        status = "ok" if validation["valid"] and aggregate["ok_runs"] > 0 else "error"
        summary["libraries"][library] = {
            "status": status,
            "execution_mode": mode,
            "runner_command": runner_command,

...
struct BenchmarkConfig {
    ...
    budget: BudgetSpec,
    seeds: Vec<u64>,
    roma_swarm_size: usize,
    roma_inertia_weight: f64,
    roma_cognitive_coefficient: f64,
    roma_social_coefficient: f64,
    roma_velocity_clamp: f64,
    roma_threads: usize,
}
...
fn load_instance(path: &Path) -> Result<BenchmarkInstance, String> {
    ...
    let weights = weights_str
        ...
        .map(|s| {
            s.parse::<f64>()
                .map_err(|error| format!("invalid weight '{}' in {}: {}", s, path.display(), error))
        })
    ...
    let values = values_str
        ...
        .map(|s| {
            s.parse::<f64>()
                .map_err(|error| format!("invalid value '{}' in {}: {}", s, path.display(), error))
        })
        .collect::<Result<Vec<f64>, String>>()?;
...
fn load_config(path: &Path) -> Result<BenchmarkConfig, String> {
    ...
    Ok(BenchmarkConfig {
        ...
        seeds,
        roma_swarm_size: required_json_parsed(path, "roma.swarm_size")?,
        roma_inertia_weight: required_json_parsed(path, "roma.inertia_weight")?,
        roma_cognitive_coefficient: required_json_parsed(path, "roma.cognitive_coefficient")?,
        roma_social_coefficient: required_json_parsed(path, "roma.social_coefficient")?,
        roma_velocity_clamp: required_json_parsed(path, "roma.velocity_clamp")?,
        roma_threads: required_json_parsed(path, "roma.threads")?,
    })
}
...
fn run_once(instance: &BenchmarkInstance, config: &BenchmarkConfig, seed: u64) -> BenchmarkResult {
    ...
    let measured: Result<(Duration, (f64, Vec<u8>)), String> = measure_result(|| {
        ...
        let parameters = PSOParameters::new(
            config.roma_swarm_size,
            config.roma_inertia_weight,
            config.roma_cognitive_coefficient,
            config.roma_social_coefficient,
            TerminationCriteria::new(vec![TerminationCriterion::MaxEvaluations(config.budget.value)]),
        )
        .with_velocity_clamp(config.roma_velocity_clamp)
        .with_threads(config.roma_threads)
        .with_seed(seed);
...
fn validate_config(instance: &BenchmarkInstance, config: &BenchmarkConfig) -> Result<(), String> {
    ...
    if config.budget.r#type != "evaluations" {
        return Err("Knapsack PSO benchmark runner currently supports only evaluation budgets".to_string());
    }
    ...
    if config.budget.value < config.roma_swarm_size {
        return Err("evaluation budget must be at least the PSO swarm size".to_string());
    }

    if config.budget.value % config.roma_swarm_size != 0 {
        return Err("evaluation budget must be divisible by the PSO swarm size".to_string());
    }

...
JAVA_SOURCE = ROOT / "runners" / "java" / "Zdt1NsgaIIBenchmark.java"
CACHE_DIR = ROOT / ".cache" / "jmetal_java"
LIB_DIR = CACHE_DIR / "lib"
CLASS_DIR = CACHE_DIR / "classes"
MAIN_CLASS = "Zdt1NsgaIIBenchmark"
...
def run_once(seed, jars):
    ...
    completed = run_command(
        [
            ...
            str(JMETAL_CONFIG["offspring_population_size"]),
            str(JMETAL_CONFIG["crossover_probability"]),
            str(JMETAL_CONFIG["mutation_probability"]),
            str(JMETAL_CONFIG["sbx_distribution_index"]),
            str(JMETAL_CONFIG["polynomial_distribution_index"]),
...
if __name__ == "__main__":
    if BUDGET.get("type") != "evaluations":
        raise ValueError("This jMetal Java benchmark runner supports only evaluation budgets")

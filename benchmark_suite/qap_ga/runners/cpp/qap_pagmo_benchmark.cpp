...
qap_data read_qap(const std::string &path)
{
    std::ifstream input(path);
    if (!input) {
        throw std::runtime_error("failed to open QAP instance: " + path);
    }

    std::size_t dimension = 0u;
    input >> dimension;
    if (!input || dimension == 0u) {
        throw std::runtime_error("invalid QAP dimension in: " + path);
    }

    qap_data data{
        std::vector<std::vector<double>>(dimension, std::vector<double>(dimension, 0.0)),
        std::vector<std::vector<double>>(dimension, std::vector<double>(dimension, 0.0)),
    };

    for (std::size_t row = 0; row < dimension; ++row) {
        for (std::size_t column = 0; column < dimension; ++column) {
            input >> data.flow_matrix[row][column];
        }
    }

    for (std::size_t row = 0; row < dimension; ++row) {
        for (std::size_t column = 0; column < dimension; ++column) {
            input >> data.distance_matrix[row][column];
        }
    }

    if (!input) {
        throw std::runtime_error("QAP file did not contain the expected matrix entries");
    }

    return data;
}

std::vector<std::size_t> decode_assignment(const pagmo::vector_double &keys)
{
    std::vector<std::size_t> assignment(keys.size());
    std::iota(assignment.begin(), assignment.end(), 0u);
    std::stable_sort(assignment.begin(), assignment.end(), [&keys](std::size_t lhs, std::size_t rhs) {
...
{
    double total = 0.0;
    for (std::size_t facility_i = 0; facility_i < assignment.size(); ++facility_i) {
        const auto location_i = assignment[facility_i];
        for (std::size_t facility_j = 0; facility_j < assignment.size(); ++facility_j) {
            const auto location_j = assignment[facility_j];
...
struct qap_random_keys_problem {
    qap_random_keys_problem() = default;

    explicit qap_random_keys_problem(qap_data data)
        : m_flow_matrix(std::move(data.flow_matrix)),
          m_distance_matrix(std::move(data.distance_matrix))
    {
    }

    pagmo::vector_double fitness(const pagmo::vector_double &x) const
    {
        const auto assignment = decode_assignment(x);
        return {qap_cost(assignment, m_flow_matrix, m_distance_matrix)};
    }

    std::pair<pagmo::vector_double, pagmo::vector_double> get_bounds() const
    {
        const auto dimension = m_flow_matrix.size();
        return {
    ...
    {
        return "QAP Random Keys";
    }

private:
    std::vector<std::vector<double>> m_flow_matrix;
...
{
    if (argc != 18) {
        std::cerr << "Usage: <benchmarkId> <algorithmFamily> <problem> <instanceId> <qapPath> <budgetType> <budgetValue> <populationSize> <crossoverProbability> <mutationProbability> <etaC> <paramM> <selection> <crossover> <mutation> <selectionParam> <seed>\n";
        return 1;
    }
    ...
    try {
        ...
        const std::string qap_path = argv[5];
        ...
        const auto data = read_qap(qap_path);
        const auto generations = generations_from_budget(budget_value, population_size);

        pagmo::problem problem{qap_random_keys_problem(data)};
        ...
        const auto best_assignment = decode_assignment(best_keys);
        ...
        std::cout << "  \"best_solution\": " << format_usize_vector(best_assignment) << ",\n";

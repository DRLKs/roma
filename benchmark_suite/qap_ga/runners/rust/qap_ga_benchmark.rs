...
use roma_lib::problem::QapProblem;
...
fn load_instance(path: &Path) -> Result<BenchmarkInstance, String> {
    let flow_matrix = get_json_number_matrix(path, "flow_matrix")
        .map_err(|error| format!("failed to read flow_matrix from {}: {}", path.display(), error))?;
...
fn build_problem(instance: &BenchmarkInstance) -> QapProblem {
    QapProblem::with_matrices(instance.flow_matrix.clone(), instance.distance_matrix.clone())
}
...
fn validate_config(instance: &BenchmarkInstance, config: &BenchmarkConfig) -> Result<(), String> {
    ...
    if config.budget.r#type != "evaluations" {
        return Err("QAP benchmark runner currently supports only evaluation budgets".to_string());
    }
    ...
    if instance.flow_matrix.len() != instance.dimension || instance.distance_matrix.len() != instance.dimension {
        return Err("flow_matrix and distance_matrix must match instance dimension".to_string());
    }

    if instance.flow_matrix.iter().any(|row| row.len() != instance.dimension) {
        return Err("flow_matrix must be square".to_string());
    }

    if instance.distance_matrix.iter().any(|row| row.len() != instance.dimension) {

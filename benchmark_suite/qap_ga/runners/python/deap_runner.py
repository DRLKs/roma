...
if not hasattr(creator, "QapFitnessMin"):
    creator.create("QapFitnessMin", base.Fitness, weights=(-1.0,))

if not hasattr(creator, "QapIndividual"):
    creator.create("QapIndividual", list, fitness=creator.QapFitnessMin)
...
def evaluate(individual):
    return (qap_cost(individual),)


def build_toolbox():
    toolbox = base.Toolbox()
    toolbox.register("permutation", random.sample, range(DIMENSION), DIMENSION)
    toolbox.register("individual", tools.initIterate, creator.QapIndividual, toolbox.permutation)

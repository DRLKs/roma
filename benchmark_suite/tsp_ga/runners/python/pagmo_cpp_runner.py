...
CPP_SOURCE = ROOT / "runners" / "cpp" / "tsp_pagmo_benchmark.cpp"
CACHE_DIR = ROOT / ".cache" / "pagmo_cpp"
BINARY_PATH = CACHE_DIR / "tsp_pagmo_benchmark"
...
def benchmark_args(seed):
    return [
        ...
        str(PAGMO_CONFIG["param_m"]),
        str(PAGMO_CONFIG["selection"]),
        str(PAGMO_CONFIG["crossover"]),
        str(PAGMO_CONFIG["mutation"]),
        str(PAGMO_CONFIG["selection_param"]),
        str(seed),
    ]

...
use roma_lib::problem::AckleyProblem;
...
fn load_config(path: &Path) -> Result<BenchmarkConfig, String> {
    ...
    Ok(BenchmarkConfig {
        ...
        crossover_rate: required_json_parsed(path, "roma.crossover_rate")?,
        differential_weight: required_json_parsed(path, "roma.differential_weight")?,
    })
}

fn build_problem(instance: &BenchmarkInstance) -> AckleyProblem {
    AckleyProblem::new(instance.dimension, instance.lower_bound, instance.upper_bound)
}
...
fn validate_config(instance: &BenchmarkInstance, config: &BenchmarkConfig) -> Result<(), String> {
    ...
    if config.budget.r#type != "evaluations" {
        return Err("Ackley benchmark runner currently supports only evaluation budgets".to_string());
    }
    ...
    if config.population_size < 4 {
        return Err("roma.population_size must be at least 4".to_string());
    }

    if !(0.0..=1.0).contains(&config.crossover_rate) {
        return Err("roma.crossover_rate must be in [0,1]".to_string());
    }

    if config.differential_weight <= 0.0 {
        return Err("roma.differential_weight must be > 0".to_string());
    }

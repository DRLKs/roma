...
IN_CONTAINER_ENV = "ROMA_ACKLEY_DE_IN_CONTAINER"
DOCKER_IMAGE = "roma-ackley-de:latest"
CONTAINER_RESULTS_DIR = "/workspace/benchmark_suite/ackley_de/results"
...
def build_runners():
    if os.environ.get(IN_CONTAINER_ENV) == "1":
        roma_command = ["/usr/local/bin/roma_ackley_de_benchmark"]
    else:
    ...
    return {
        ...
        "scipy": [sys.executable, str(RUNNERS_DIR / "python" / "scipy_runner.py")],
        "random_search": [sys.executable, str(RUNNERS_DIR / "python" / "random_search_runner.py")],
...
def validate_results(results):
    ...
    for index, result in enumerate(results):
        ...
        recomputed = ackley_value(solution)
